...
import { Brain, Mail, Lock, User, Eye, EyeOff, Github, CheckCircle2, Zap, BarChart3, Shield } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const PERKS = [
    { icon: Zap,       text: 'Real-time AI feedback on every answer' },
    { icon: BarChart3, text: 'Detailed performance analytics & reports' },
    { icon: Shield,    text: 'FAANG-level adaptive question engine' },
  ];
    ...
    <div className="min-h-screen flex" style={{ background: 'var(--bg-page)' }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] p-12 relative overflow-hidden"
        ...
        <div className="relative">
          ...
          <p className="text-white/70 text-sm leading-relaxed max-w-xs mb-10">
            Join 50,000+ engineers who ace their interviews with AI-powered practice.
          </p>
        ...
        <div className="relative">
          <p className="text-xs text-white/50 mb-3">Trusted by engineers at</p>
          <div className="flex flex-wrap gap-2">
            {['Google', 'Amazon', 'Meta', 'Microsoft', 'Apple'].map(c => (
              <span key={c} className="px-3 py-1 rounded-full text-xs font-medium bg-white/15 text-white/80">{c}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-[var(--text-muted)] mb-8">Start practicing for free today — no credit card needed</p>
          ...
          <div className="mt-5 flex items-start gap-2 p-3 rounded-xl" style={{ background: 'var(--success-light)', border: '1px solid #bbf7d0' }}>
            <CheckCircle2 className="w-4 h-4 text-[var(--success)] shrink-0 mt-0.5" />
            <p className="text-xs text-[var(--text-secondary)]">
              5 free interviews every month. No credit card required.
            </p>
          </div>
